...
export default function Page() {
  return (
    <>
      ...
      <Lede>
        The core loop: connect a SIEM, pick an attack technique, run it against a real endpoint, and see &mdash;
        with evidence &mdash; whether your detection actually fired.
      </Lede>

      <H2 id="connect-a-siem">Connect a SIEM</H2>
      <P>
        <strong>Settings &rarr; SIEM.</strong> PurveX reads from your SIEM at validation time &mdash; it never
        copies or stores your event data. Three types are supported today:
      </P>
      <Table
      ...
      <P>
        ...
        project, organized strictly by MITRE ATT&amp;CK technique &mdash; one card per tactic, with a
        &ldquo;Has detections&rdquo; / &ldquo;No detections&rdquo; chip on each technique. First time you open a
        technique with no library installed yet, you&apos;ll see a <strong>Download test catalog</strong> button
        &mdash; one click, cached after that.
      </P>
      <Callout tone="info">
        Opening a test shows you the exact command it runs, but PurveX doesn&apos;t execute anything from this
        screen &mdash; it&apos;s reference. Clicking <strong>Run</strong> hands the technique off to the Run Test
      ...
      <P>
        PurveX watches the run through four stages &mdash; Queued, Running, Ingesting, Validated &mdash; then scores
        it 0&ndash;100 based on whether logs arrived, how fast, whether the rule fired, and how fast it fired.
      </P>
      <Table
      ...
      <P>
        ...
        exact command that ran, and a sample of the raw SIEM events PurveX pulled back &mdash; not just a pass/fail
        badge. If Watchtower is configured, you&apos;ll also get a plain-language explanation and a suggested fix.
      </P>

      <H2 id="coverage">See your coverage</H2>
      <P>
        <strong>MITRE Coverage</strong> (top-level nav). A heatmap &mdash; one column per tactic, one card per
