...
// shows one at a time with Back / Next below it. Lessons with fewer than
// two missions are left alone.
export function MissionPager({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (total >= 2) missions()[at]?.scrollIntoView({ block: "start", behavior: "smooth" });
  }, [at, total, missions]);
  ...
  return (
    <div ref={root}>
      {children}
      {total >= 2 && (
        <nav className="ad-pager" aria-label="Questions">
          <button type="button" className="ad-pager__btn" disabled={at === 0} onClick={() => setStep(at - 1)}>
            <ArrowLeft className="h-4 w-4" /> Back
          </button>
          <div className="ad-pager__mid">
            <span className="ad-pager__count">
              Question {at + 1} of {total}
            </span>
            <span className="ad-pager__dots">
              {solved.map((done, i) => (
                <button
                  key={i}
                  type="button"
                  ...
                  aria-current={i === at}
                  className={`ad-pager__dot${done ? " is-done" : ""}${i === at ? " is-here" : ""}`}
                  onClick={() => setStep(i)}
                />
              ))}
            </span>
