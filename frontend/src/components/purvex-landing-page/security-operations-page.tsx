...
.sp-console__icon { flex-shrink: 0; width: 40px; height: 40px; border-radius: var(--radius); background: transparent; border: 1px solid var(--border-strong); color: var(--accent-deep); display: flex; align-items: center; justify-content: center }
...
/* ── Blue vs red (inline version): divided strip, matches the site's card convention ── */
.sp-versus {
  display: grid; grid-template-columns: 1fr auto 1fr; align-items: center; gap: 20px;
  max-width: 640px; margin: 0 auto;
  border-top: 1px solid var(--border); border-bottom: 1px solid var(--border);
}
.sp-versus__side { padding: 28px 26px; text-align: center }
.sp-versus__icon {
  display: inline-flex; align-items: center; justify-content: center;
  width: 44px; height: 44px; border-radius: 50%; margin-bottom: 12px;
  border: 1px solid transparent;
}
.sp-versus__side--blue .sp-versus__icon { background: rgba(37,99,235,.1); border-color: rgba(37,99,235,.18); color: #2563eb }
.sp-versus__side--red .sp-versus__icon { background: rgba(229,72,77,.1); border-color: rgba(229,72,77,.18); color: var(--red) }
