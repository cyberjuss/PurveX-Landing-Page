...
A port tells you *which door* traffic is using. A protocol tells you *what is actually happening* once it is through that door, and the two ideas are easy to conflate until you have seen enough traffic to feel the difference.
...
* **Fundamental:** A port narrows traffic down to a category, web, mail, name lookups. The protocol is the actual conversation happening inside that category. Knowing both is what separates reading a packet capture from staring at a list of numbers.
