...
function handsOnLine(results: Results, lab: LabSnapshot | null): string {
  ...
  const parts = [
    ...
    ev.ctfPlanted ? "Ticket-queue CTF objects are planted." : ticketsTouched ? "Tickets were attempted but CTF objects are missing — they still need to run the linked script from Build the Environment again." : "CTF objects not planted yet.",
  ];
...
export async function runCoachTool(name: string, input: Record<string, unknown>, ctx: CoachToolContext): Promise<string> {
  ...
  if (name === "get_lab_findings") {
    const lab = await ctx.loadLabState();
    if (!lab) return JSON.stringify({ connected: false, note: "No lab snapshot yet. Ask the student to run Build-Environment.ps1 from Build the Environment." });
  ...
  if (name === "get_event_digest") {
    const lab = await ctx.loadLabState();
    if (!lab?.events) return JSON.stringify({ available: false, note: "The lab has not sent a Security log digest. The student needs the latest lab script from Build the Environment, and the log needs some recent activity." });
